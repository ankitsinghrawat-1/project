...
app.put('/api/campaigns/:id', async (req, res) => {
    ...
    try {
        await pool.query(
            ...
            [DOMPurify.sanitize(title), DOMPurify.sanitize(description), goal_amount, start_date, end_date, image_url, req.params.id]
        );
...
app.post('/api/jobs/:job_id/apply', upload.single('resume'), async (req, res) => {
    ...
    try {
        await pool.query(
            ...
            [job_id, email, full_name, resume_path, DOMPurify.sanitize(cover_letter)]
        );
...
app.get('/api/admin/applications', async (req, res) => {
    try {
        const [rows] = await pool.query(`
            SELECT 
                ja.application_id, ja.full_name, ja.user_email, ja.resume_path, ja.application_date, j.title AS job_title
...
app.post('/api/login', loginLimiter, async (req, res) => {
    ...
    try {
        ...
        if (isMatch) {
            res.cookie('loggedIn', 'true', { httpOnly: true, secure: true, sameSite: 'lax', maxAge: 3600000 });
...
app.post('/api/signup', async (req, res) => {
    ...
    try {
        ...
        await pool.query('INSERT INTO users (full_name, email, password_hash) VALUES (?, ?, ?)', [DOMPurify.sanitize(full_name), email, password_hash]);
...
app.post('/api/onboard', async (req, res) => {
    ...
    try {
        await pool.query(
            ...
            [DOMPurify.sanitize(university), university_email, DOMPurify.sanitize(city), DOMPurify.sanitize(graduation_year), DOMPurify.sanitize(major), DOMPurify.sanitize(degree), DOMPurify.sanitize(current_company), DOMPurify.sanitize(job_title), DOMPurify.sanitize(bio), linkedin, email]
        );
...
app.put('/api/events/:id', async (req, res) => {
    ...
    try {
        await pool.query(
            ...
            [DOMPurify.sanitize(title), DOMPurify.sanitize(description), date, DOMPurify.sanitize(location), DOMPurify.sanitize(organizer), req.params.id]
        );
...
app.put('/api/jobs/:id', async (req, res) => {
    ...
    try {
        await pool.query(
            ...
            [DOMPurify.sanitize(title), DOMPurify.sanitize(description), DOMPurify.sanitize(company), DOMPurify.sanitize(location), contact_email, req.params.id]
        );
...
app.post('/api/jobs', async (req, res) => {
    ...
    try {
        await pool.query('INSERT INTO jobs (title, company, location, description, contact_email) VALUES (?, ?, ?, ?, ?)', [DOMPurify.sanitize(title), DOMPurify.sanitize(company), DOMPurify.sanitize(location), DOMPurify.sanitize(description), contact_email]);
...
app.post('/api/events', async (req, res) => {
    ...
    try {
        await pool.query('INSERT INTO events (title, date, location, organizer, description) VALUES (?, ?, ?, ?, ?)', [DOMPurify.sanitize(title), date, DOMPurify.sanitize(location), DOMPurify.sanitize(organizer), DOMPurify.sanitize(description)]);
